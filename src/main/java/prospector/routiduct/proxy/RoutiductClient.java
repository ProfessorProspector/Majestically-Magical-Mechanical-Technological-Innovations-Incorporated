...
public class RoutiductClient extends RoutiductServer {
	...
	private static void register(Item item, int meta, String name) {
		ModelLoader.setCustomModelResourceLocation(item, meta,
			new ModelResourceLocation(Routiduct.PREFIX + name, "inventory"));
	}
	...
	private static void registerBlockstate(Item i, int meta, String variant, String dir) {
		ResourceLocation loc = new ResourceLocation(Routiduct.MOD_ID, dir + i.getRegistryName().getResourcePath());
	...
	private static void registerBlockstateMultiItem(Item item, String variantName, String path) {
		ResourceLocation loc = new ResourceLocation(Routiduct.MOD_ID, path);
		ModelLoader.setCustomModelResourceLocation(item, 0, new ModelResourceLocation(loc, "type=" + variantName));
	}

	@Override
	public void registerRenders() {
		for (Item item : Routiduct.MOD_CL.itemModelsToRegister) {
			registerItemModel(item, 0);
		}
		for (Block block : Routiduct.MOD_CL.blockModelsToRegister) {
